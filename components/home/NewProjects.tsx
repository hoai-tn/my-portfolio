...
const NewProjects = () => {
  ...
  const [carouselApis, setCarouselApis] = useState<{
    [key: string]: CarouselApi;
  }>({});
  ...
  const projects: Project[] = [
    ...
    },
    {
      title: "CoolPix",
      description:
        "Professional photo management and sharing platform built with modern web technologies. Features secure cloud storage with AWS S3, intuitive gallery management, and seamless photo organization capabilities. Delivers fast performance and scalable architecture for efficient media handling.",
      impact: {
        storage: "AWS S3 Integration",
        performance: "Optimized Media Delivery",
      },
      technologies: ["Vue.js", "Laravel", "MySQL", "AWS S3", "RESTful API"],
      skillCategories: {
        frontend: ["Vue.js"],
      ...
      images: [
        "/images/coolpix/coolpix-1.png",
        "/images/coolpix/coolpix-2.png",
        "/images/coolpix/coolpix-5.png",
        "/images/coolpix/coolpix-4.png",
      ],
  ...
  React.useEffect(() => {
    // Initialize carousel tracking for each project
    featuredProjects.forEach((project) => {
      if (!currentSlides[project.title]) {
        setCurrentSlides((prev) => ({ ...prev, [project.title]: 0 }));
      }
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleCarouselApi = (api: CarouselApi, projectTitle: string) => {
    if (!api) return;

    setCarouselApis((prev) => ({ ...prev, [projectTitle]: api }));

    api.on("select", () => {
      setCurrentSlides((prev) => ({
        ...prev,
        [projectTitle]: api.selectedScrollSnap(),
      }));
    });
  };
      ...
      <div className="space-y-16">
        {featuredProjects.map((project, index) => (
          ...
          >
            ...
            >
              <Card className="bg-transparent border-[var(--glass-border)] overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300">
                <CardContent className="p-0 relative">
                  <Carousel
                    ...
                    setApi={(api) => handleCarouselApi(api, project.title)}
                  >
                    <CarouselContent>
                      {project.images.map((image, imgIndex) => (
                        <CarouselItem key={imgIndex}>
                          <div className="aspect-[16/10] relative overflow-hidden bg-[var(--bg-secondary)]">
                            <Image
                              src={image}
                    ...
                    {project.images.length > 1 && (
                      <>
                        ...
                        <CarouselPrevious
                          className={cn(
                            "left-4 bg-[var(--glass-bg)]/90 backdrop-blur-sm border-[var(--glass-border)] text-[var(--text-primary)] hover:bg-[var(--accent-primary)] hover:text-black transition-all duration-300 hover:scale-110 opacity-0 group-hover:opacity-100",
                        ...
                        <CarouselNext
                          className={cn(
                            "right-4 bg-[var(--glass-bg)]/90 backdrop-blur-sm border-[var(--glass-border)] text-[var(--text-primary)] hover:bg-[var(--accent-primary)] hover:text-black transition-all duration-300 hover:scale-110 opacity-0 group-hover:opacity-100",
                            project.isLightTheme &&
                              "bg-black/70 text-white hover:bg-black/90 hover:text-white border-white/20"
                          )}
                        />
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                          {project.images.map((_, dotIndex) => (
                            <button
                              key={dotIndex}
                              onClick={() => {
                                carouselApis[project.title]?.scrollTo(dotIndex);
                              }}
                              className={cn(
                                "h-2 rounded-full transition-all duration-300",
                                dotIndex === (currentSlides[project.title] || 0)
                        ...
                        <div className="absolute top-0 left-0 right-0 h-1 bg-white/10">
                          <div
                            ...
                            style={{
                              width: `${
                                (((currentSlides[project.title] || 0) + 1) /
                        ...
                        <div
                          className={cn(
                            "absolute top-4 right-4 bg-[var(--glass-bg)]/90 backdrop-blur-sm border border-[var(--glass-border)] px-3 py-1 rounded-full text-xs font-mono text-[var(--text-primary)]",
                            project.isLightTheme &&
